...
import com.dragon.gerrit.jenson.data.predicates.ByExactSeverityPredicate;
...
public class CustomReportFormatter implements TagFormatter<CustomReportFormatter.Tag> {
    ...
    private Iterable<ScrIssue> issues;

    public CustomReportFormatter(Iterable<ScrIssue> issues, String failMessage, String successMessage) {
        this.issues = issues;
    ...
    private Iterable<ScrIssue> filterByExactSeverityPredicate(Severity s) {
        return Iterables.filter(issues, ByExactSeverityPredicate.apply(s));
    }

    private Iterable<ScrIssue> filterByMinSeverityPredicate(Severity s) {
